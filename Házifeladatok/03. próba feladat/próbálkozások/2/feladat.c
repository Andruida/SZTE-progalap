...
int huszar(char oszlop, char sor) {
    // printf("%d %d", oszlop - 'a', sor - '1');
    int x = oszlop - 'a';
    int y = sor - '1';
    int ut = 0;
    for (int i = 1; i <= 2; i++) {                  // X tengelyen egyet vagy kettőt lépjünk
        for (int n1 = -1; n1 < 2; n1 += 2) {        // Jobbra vagy balra lépünk az X tengelyen
            for (int n2 = -1; n2 < 2; n2 += 2) {    // Fel vagy le lépünk az Y tengelyen
                int cx = i*n1;
                int cy = (!(i-1)+1) * n2;            // Ha kettőt léptünk az X tengelyen, itt csak egyet léphetünk
                if (0 <= x + cx && x + cx <= 7 && 0 <= y + cy && y + cy <= 7) {
                    ut++;
                }
            }
        }
...
int evnapja(int ev, int ho, int nap) {
    ...
    if ((ev % 4 == 0 && ev % 100 != 0) || (ev % 100 == 0 && ev % 400 == 0 ) && (ho > 2)) {
        napok++;
    }
...
int erme(int osszeg) {
    ...
    int denoms[] = {10, 5, 2, 1};
    for (int i = 0; i < 4; i++) {
        int currCoins = left / denoms[i];
        left -= currCoins * denoms[i];
        coins += currCoins;
    }
    return coins;
}
