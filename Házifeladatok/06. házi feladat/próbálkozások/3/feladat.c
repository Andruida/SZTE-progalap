...
double elem(double elso, double masodik, double harmadik, int n) {
    switch (n)
    {
    case 1:
        return elso;
    ...
    double d1 = masodik-elso;
    double d2 = harmadik-masodik;
    double q = d2/d1;
    double prev = elem(elso, masodik, harmadik, n-1);
    return elem(elso, masodik, harmadik, n-1) + d1 * hatvany(q, n-2);
}

double sorozat(double elso, double masodik, double harmadik, int n) {
    double sum = 0;
    for (int i = 1; i <= n; i++) {
        sum += elem(elso, masodik, harmadik, i);
    }
    return sum;
}
